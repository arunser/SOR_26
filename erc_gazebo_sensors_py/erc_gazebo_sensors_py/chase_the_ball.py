...
class ImageSubscriber(Node):
    ...
    #     # Create all four OpenCV windows
    #     cv2.namedWindow("frame", cv2.WINDOW_NORMAL)
    #     cv2.namedWindow("mask", cv2.WINDOW_NORMAL)
    #     cv2.namedWindow("contour", cv2.WINDOW_NORMAL)
    #     cv2.namedWindow("crosshair", cv2.WINDOW_NORMAL)
        
    #     # Optional: Resize them so they don't take up your entire screen at once
    #     cv2.resizeWindow("frame", 400, 300)
    #     cv2.resizeWindow("mask", 400, 300)
    #     cv2.resizeWindow("contour", 400, 300)
    #     cv2.resizeWindow("crosshair", 400, 300)

    #     while rclpy.ok():
    #         # Check if there is a new frame available
    #         if self.latest_frame is not None:

    #             # 1. Capture the three masks returned by your processing function
    #             red_mask, contour_mask, crosshair_mask = self.process_image(self.latest_frame)

    #             # 2. Display all four windows
    #             cv2.imshow("frame", self.latest_frame)
    #             cv2.imshow("mask", red_mask)
    #             cv2.imshow("contour", contour_mask)
    #             cv2.imshow("crosshair", crosshair_mask)
